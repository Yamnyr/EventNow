...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $usersData = [
            ['email' => 'user1@example.com', 'roles' => ['ROLE_USER'], 'date_naissance' => new \DateTime('1990-01-01'), 'nom' => 'User 1'],
            ['email' => 'user2@example.com', 'roles' => ['ROLE_USER'], 'date_naissance' => new \DateTime('1995-05-10'), 'nom' => 'User 2'],
        ...
        $inscriptionsData = [
            ['utilisateur' => $this->getReference('user_user1@example.com'), 'date' => $this->getReference('date_Concert Rock'), 'nombre_personnes' => 2, 'age_requis' => true, 'date_inscription' => new \DateTime(), 'uuid' => '123456789'],
            // Add more inscriptions if needed
        ];

        foreach ($inscriptionsData as $inscriptionData) {
            $inscription = new Inscription();
            $inscription->setUtilisateur($inscriptionData['utilisateur']);
            $inscription->setDate($inscriptionData['date']);
            $inscription->setNombrePersonnes($inscriptionData['nombre_personnes']);
            $inscription->setAgeRequis($inscriptionData['age_requis']);
